...
function FloatingActivityFeed() {
  const logs = [
    "Analyzing competitor positioning...",
    "Optimizing conversion structure...",
    "Audience heatmap updated...",
    "Brand trust signals increasing...",
    "Scanning engagement metrics...",
  ];

  return (
    <div className="pointer-events-none absolute right-6 top-6 z-30 hidden w-72 overflow-hidden rounded-[1.6rem] border border-white/10 bg-white/10 p-4 backdrop-blur-2xl xl:block">
      <div className="mb-3 flex items-center gap-2 text-xs font-black tracking-[0.25em] text-cyan-200">
        <div className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_14px_rgba(74,222,128,0.9)]" />
        LIVE AI ACTIVITY
      </div>
      <div className="space-y-2">
        {logs.map((log, i) => (
          <motion.div
            ...
            className="rounded-xl border border-white/5 bg-black/20 px-3 py-2 text-xs font-semibold text-white/70"
          >
            {log}
          </motion.div>
        ))}
...
function NoiseTexture() {
  return (
    <div
      ...
      style={{
        backgroundImage:
          "url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"120\" height=\"120\" viewBox=\"0 0 120 120\"><filter id=\"n\"><feTurbulence type=\"fractalNoise\" baseFrequency=\"1.1\" numOctaves=\"2\" stitchTiles=\"stitch\"/></filter><rect width=\"120\" height=\"120\" filter=\"url(%23n)\" opacity=\"0.9\"/></svg>')",
      }}
    />
  );
}
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 px-6 pb-24 pt-16 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="relative min-h-[36rem] overflow-visible rounded-[2rem] border border-black/10 bg-white/70 p-5 shadow-2xl shadow-violet-100/70 backdrop-blur-xl">
              ...
              <div className="relative grid min-h-[34rem] gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                <motion.div
                  whileHover={{ scale: 1.04, rotate: -1, filter: "blur(0px)" }}
                  animate={{ y: [0, -14, 0], x: [0, 8, 0] }}
                  transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
                  className="absolute -left-8 top-10 z-30 hidden w-56 rounded-[1.5rem] border border-white/20 bg-white/15 p-4 shadow-[0_0_45px_rgba(139,92,246,0.22)] backdrop-blur-2xl lg:block"
                >
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-xs font-black tracking-[0.22em] text-violet-200">AI PANEL</p>
                    <div className="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_15px_rgba(74,222,128,0.9)]" />
                  </div>
                  <div className="space-y-2">
                    <div className="h-2 w-4/5 rounded-full bg-white/30" />
                    <div className="h-2 w-2/3 rounded-full bg-cyan-300/40" />
                    <div className="h-2 w-5/6 rounded-full bg-violet-300/40" />
                  </div>
                </motion.div>
                ...
                <motion.div
                  ...
                  className="absolute -right-10 bottom-20 z-30 hidden w-60 rounded-[1.5rem] border border-white/20 bg-white/15 p-4 shadow-[0_0_45px_rgba(34,211,238,0.2)] backdrop-blur-2xl lg:block"
                >
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-xs font-black tracking-[0.22em] text-cyan-200">COMMAND</p>
                    <Sparkles size={16} className="text-cyan-200" />
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    {[45, 70, 55, 90, 62, 82, 48, 75].map((height, i) => (
                      <motion.div
                        key={`mini-bar-${i}`}
                        animate={{ height: [height - 12, height, height - 8] }}
                        transition={{ duration: 1.8 + i * 0.1, repeat: Infinity, ease: "easeInOut" }}
